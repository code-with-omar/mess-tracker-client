...
const usePerUserDetails = ({ user }) => {
  ...
  //   console.log(users);
  // Calculate totals
  let totalUserDeposit =
    usersDeposit?.reduce((acc, item) => acc + parseFloat(item.money || 0), 0) ||
    0;
  const totalMeal =
    allMeals?.reduce((acc, item) => acc + parseFloat(item.meal || 0), 0) || 0;
  const totalBazar =
    bazar?.reduce((acc, item) => acc + parseFloat(item.bazar || 0), 0) || 0;
  const extraBazar =
    bazar?.reduce((acc, item) => acc + parseFloat(item.extra || 0), 0) || 0;
